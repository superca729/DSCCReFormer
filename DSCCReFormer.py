...
class TSAttention(nn.Module):
    def __init__(
            self, dim, s_attn_size, t_attn_size, sem_num_heads=2, t_num_heads=2, qkv_bias=False,
            attn_drop=0., device=torch.device('cpu'), rmsnorm=True):
        super().__init__()
        # assert dim % (sem_num_heads + t_num_heads) == 0
        self.sem_num_heads = sem_num_heads
        self.t_num_heads = t_num_heads
        self.head_dim = 2 * dim // (sem_num_heads + t_num_heads)
        self.scale = self.head_dim ** -0.5
        self.device = device
        self.s_attn_size = s_attn_size
        self.t_attn_size = t_attn_size
        self.sem_ratio = sem_num_heads / (sem_num_heads + t_num_heads)
        self.t_ratio = 1 - self.sem_ratio

        self.sem_q_conv = nn.Conv2d(dim, dim, kernel_size=1, bias=qkv_bias)
        self.sem_k_conv = nn.Conv2d(dim, dim, kernel_size=1, bias=qkv_bias)
        self.sem_v_conv = nn.Conv2d(dim, dim, kernel_size=1, bias=qkv_bias)
        ...
        # self.t_sep_conv = nn.Conv2d(int(dim * self.t_ratio), int(dim * self.t_ratio), kernel_size=3, stride=1,
        #                             padding=1, groups=dim)
        # self.sem_sep_conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1,
        #                               padding=1, groups=dim)
        #
        # self.st_norm = RMSNorm(dim) if rmsnorm else nn.LayerNorm(dim)
        # self.geo_norm = RMSNorm(dim) if rmsnorm else nn.LayerNorm(dim)

    def forward(self, t_x, sem_x, sem_mask=None):
        B, T, N, D = t_x.shape
        t_q = self.t_q_conv(t_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
        t_k = self.t_k_conv(t_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
        t_v = self.t_v_conv(t_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)

        # sep_t = self.t_sep_conv(t_v.permute(0, 3, 2, 1)).permute(0, 2, 3, 1)
        ...
        # t_attn = F.relu(t_attn)
        t_attn = self.t_attn_drop(t_attn)
        t_x = (t_attn @ t_v).transpose(2, 3).reshape(B, N, T, D).transpose(1, 2)
        # t_attn = t_attn.softmax(dim=-1)
        # t_x = self.st_norm(t_x)
        # t_x = t_x + sep_t

        sem_q = self.sem_q_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        sem_k = self.sem_k_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        sem_v = self.sem_v_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

        # sep_sem = self.sem_sep_conv(sem_v.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
        ...
        # sem_attn = F.relu(sem_attn)
        sem_attn = self.sem_attn_drop(sem_attn)
        sem_x = (sem_attn @ sem_v).transpose(2, 3).reshape(B, T, N, D)
        # sem_x = self.st_norm(sem_x)
        # sem_x = sem_x + sep_sem
        return t_x, sem_x
...
class STSelfAttention(nn.Module):
    def __init__(
            ...
            attn_drop=0., proj_drop=0., device=torch.device('cpu'), output_dim=1, mode="stage_para", rmsnorm=True
    ):
        super().__init__()
        ...
        self.q_conv = nn.Conv2d(int(dim * self.t_ratio), int(dim * self.t_ratio), kernel_size=1, bias=qkv_bias)
        self.k_conv = nn.Conv2d(int(dim * self.t_ratio), int(dim * self.t_ratio), kernel_size=1, bias=qkv_bias)
        self.v_conv = nn.Conv2d(int(dim * self.t_ratio), int(dim * self.t_ratio), kernel_size=1, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)

        self.geo_sep_conv = nn.Conv2d(int(dim * self.geo_ratio), int(dim * self.geo_ratio), kernel_size=3, stride=1,
                                      padding=1, groups=int(dim * self.geo_ratio))
        self.sem_sep_conv = nn.Conv2d(int(dim * self.sem_ratio), int(dim * self.sem_ratio), kernel_size=3, stride=1,
                                      padding=1, groups=int(dim * self.sem_ratio))
        self.t_sep_conv = nn.Conv2d(int(dim * self.t_ratio), int(dim * self.t_ratio), kernel_size=3, stride=1,
                                    padding=1, groups=int(dim * self.t_ratio))

        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

        self.ts_attn = TSAttention(int(dim * self.t_ratio), s_attn_size, t_attn_size, sem_num_heads, t_num_heads,
                                   qkv_bias,
                                   attn_drop, device=device)

        self.up_conv = nn.Conv2d(int(dim * self.t_ratio), dim, kernel_size=1, bias=qkv_bias)

        self.mode = mode

        self.ln1 = nn.LayerNorm(int(dim * self.t_ratio))

        self.sem_norm = RMSNorm(int(dim * self.sem_ratio)) if rmsnorm else nn.LayerNorm(int(dim * self.sem_ratio))
        self.t_norm = RMSNorm(int(dim * self.t_ratio)) if rmsnorm else nn.LayerNorm(int(dim * self.t_ratio))
        self.geo_norm = RMSNorm(int(dim * self.geo_ratio)) if rmsnorm else nn.LayerNorm(int(dim * self.geo_ratio))

    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        B, T, N, D = x.shape
        if self.mode == "stage_para":
            ...
            t_x = self.forward_temporal(x, t_q, t_k, t_v)
            ...
            sem_x = self.forward_spatial(x, sem_q, sem_k, sem_v, sem_mask)
            ...
            geo_x = self.forward_geographic(x, geo_q, geo_k, geo_v, x_patterns, pattern_keys, geo_mask)
            # x = self.proj_drop(torch.cat([t_x, geo_x, sem_x], dim=-1))

            t_x_tmp, sem_x_tmp = self.ts_attn(t_x=sem_x, sem_x=t_x, sem_mask=sem_mask)
            x = self.proj(torch.cat([t_x_tmp, geo_x, sem_x_tmp], dim=-1))
            x = self.proj_drop(x)
            ...
            sem_q = self.q_conv(t_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            sem_k = self.k_conv(t_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            sem_v = self.v_conv(t_x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

            sem_x = self.forward_spatial(x, sem_q, sem_k, sem_v, sem_mask)

            x = sem_x
            x = self.up_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            x = self.proj_drop(x)
            ...
            sem_v = self.sem_v_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            sem_x = self.forward_spatial(x, sem_q, sem_k, sem_v, sem_mask)
            sem_x = self.ln1(sem_x)

            t_q = self.q_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
            t_k = self.k_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
            t_v = self.v_conv(sem_x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
            ...
            x = self.up_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            x = self.proj_drop(x)
        # t_x_stage2, sem_x_stage2 = self.ts_attn(t_x=sem_x, sem_x=t_x, sem_mask=sem_mask)
        # x = self.proj(torch.cat([t_x_stage2, geo_x, sem_x_stage2], dim=-1))

        x = self.proj_drop(x)
        return x

    def forward_conv_qkv(self, x):
        q = self.q_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
        k = self.k_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
        v = self.v_conv(x.permute(0, 3, 1, 2)).permute(0, 3, 2, 1)
        return q, k, v

    def forward_temporal(self, x, t_q, t_k, t_v):
        B, T, N, D = x.shape
        ...
        # 提取 t_attn 和 t_x 的值并重塑为适合绘图的数据形状
        # t_attn_values = t_attn.view(B * N * T, -1)
        # t_x_values = t_x.reshape(B * N * T, -1)
        #
        # attention_map_np = t_attn_values.cpu().detach().numpy()
        # t_x_np = t_x_values.cpu().detach().numpy()
        # save_dir = '/root/autodl-tmp/time_series/PDFormer/vis_/attention_map'
        #
        # if not os.path.exists(save_dir):
        #     os.makedirs(save_dir)
        # np.save(os.path.join(save_dir, 't_attn_values_attention_map.npy'), attention_map_np)
        # np.save(os.path.join(save_dir, 't_x_values_attention_map.npy'), t_x_np)

        t_x = t_x + sep_t
        return t_x

    def forward_spatial(self, x, sem_q, sem_k, sem_v, sem_mask=None):
        B, T, N, D = x.shape

        sep_sem_v = sem_v
        sep_sem = self.sem_sep_conv(sep_sem_v.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
        ...
        sem_x = self.sem_norm(sem_x)

        sem_x = sem_x + sep_sem
        return sem_x

    def forward_geographic(self, x, geo_q, geo_k, geo_v, x_patterns, pattern_keys, geo_mask=None):
        B, T, N, D = x.shape
        ...
        sep_geo = self.geo_sep_conv(geo_v.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
...
class STEncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.st_attn = STSelfAttention(
            ...
            attn_drop=attn_drop, proj_drop=drop, device=device, output_dim=output_dim, mode=mode
        )
...
class PDFormer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        enc_depth = config.get("enc_depth", 8)
        ...
        #         output_dim=self.output_dim, mode="stage_para",
        #     ) for i in range(enc_depth)
        # ])

        self.num_stage1_block = 6
        self.num_stage2_block = enc_depth - self.num_stage1_block

        enc_dpr_stage1 = [x.item() for x in torch.linspace(0, drop_path, self.num_stage1_block)]
        enc_dpr_stage2 = [x.item() for x in torch.linspace(0, drop_path, self.num_stage2_block)]

        self.encoder_blocks_stage1 = nn.ModuleList([
            STEncoderBlock(
                dim=self.embed_dim, s_attn_size=self.s_attn_size, t_attn_size=self.t_attn_size,
                geo_num_heads=geo_num_heads, sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, drop_path=enc_dpr_stage1[i],
                act_layer=nn.GELU,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), device=self.device, type_ln=type_ln,
                output_dim=self.output_dim, mode="stage_para",
            ) for i in range(self.num_stage1_block)
        ])

        self.stage2_encoder_st_blocks = nn.ModuleList([
            STEncoderBlock(
                dim=self.embed_dim, s_attn_size=self.s_attn_size, t_attn_size=self.t_attn_size,
                geo_num_heads=geo_num_heads, sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, drop_path=enc_dpr_stage2[i],
                act_layer=nn.GELU,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), device=self.device, type_ln=type_ln,
                output_dim=self.output_dim, mode="stage_st",
            ) for i in range(self.num_stage2_block)
        ])
        self.stage2_encoder_ts_blocks = nn.ModuleList([
            STEncoderBlock(
                dim=self.embed_dim, s_attn_size=self.s_attn_size, t_attn_size=self.t_attn_size,
                geo_num_heads=geo_num_heads, sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, drop_path=enc_dpr_stage2[i],
                act_layer=nn.GELU,
                norm_layer=partial(nn.LayerNorm, eps=1e-6), device=self.device, type_ln=type_ln,
                output_dim=self.output_dim, mode="stage_ts",
            ) for i in range(self.num_stage2_block)
        ])
    ...
    def forward(self, batch, lap_mx=None):
        ...
        for i, encoder_block in enumerate(self.encoder_blocks_stage1):
            enc = encoder_block(enc, x_patterns, pattern_keys, self.geo_mask, self.sem_mask)
            skip += self.skip_convs[i](enc.permute(0, 3, 2, 1))

        for i in range(self.num_stage2_block):
            x_st = self.stage2_encoder_st_blocks[i](enc, x_patterns, pattern_keys, self.geo_mask, self.sem_mask)
            x_ts = self.stage2_encoder_ts_blocks[i](enc, x_patterns, pattern_keys, self.geo_mask, self.sem_mask)
            enc = x_st + x_ts
            skip += self.skip_convs[i](enc.permute(0, 3, 2, 1))
